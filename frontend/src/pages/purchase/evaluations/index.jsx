...
const FILTER_FIELDS = [
  { value: 'evaluation_id', label: 'Evaluation ID', type: 'text' },
  { value: 'rfq_no', label: 'RFQ', type: 'text' },
  { value: 'evaluation_date', label: 'Evaluation Date', type: 'date' },
  { value: 'evaluated_by_name', label: 'Evaluated By', type: 'text' },
  { value: 'recommended_vendor_name', label: 'Recommended Vendor', type: 'text' },
  { value: 'approval_status', label: 'Approval Status', type: 'select', options: [
...
export default function QuoteEvaluationPage() {
  ...
  return (
    <MainLayout>
      ...
      {activeTab === 'list' ? <EvaluationListTab /> : <EvaluationDashboardTab />}
    </MainLayout>
  );
}
...
function EvaluationListTab() {
  ...
  const columns = [
    ...
    {
      key: 'evaluation_date', label: 'Evaluation Date', sortable: true,
      render: (value) => value ? new Date(value).toLocaleDateString() : '-',
    },
    { key: 'evaluated_by', label: 'Evaluated By', sortable: true,
      render: (value, row) => row.evaluated_by_name || '-',
    },
    { key: 'recommended_vendor_name', label: 'Recommended Vendor', sortable: true, render: (value, row) => row.recommended_vendor ? (
      <button onClick={(e) => { e.stopPropagation(); navigate(`/masters/vendors/${row.recommended_vendor}`); }} className="text-primary-600 hover:text-primary-800 hover:underline font-medium">{value || '-'}</button>
    ) : (value || '-') },
    {
  ...
  return (
    <>
      <div className="flex justify-end mb-4">
        <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 px-3 py-1.5 text-sm text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50 transition">
      ...
      <DataTable exportFileName="quote-evaluations" columns={columns} data={filteredData} onRowClick={(row) => navigate(`/purchase/evaluations/${row.id}`)} />
    </>
  );
}
...
function EvaluationDashboardTab() {
  ...
  useEffect(() => {
    setPrLoading(true);
    apiClient.get('/api/purchase/requests/', { params: { page_size: 500 } })
      .then(res => setPrRaw(res.data?.results || res.data || []))
      .catch(() => setPrRaw([]))
    ...
    if (apiComparison && apiComparison.products && apiComparison.products.length > 0) {
        ...
        (p.vendor_prices || []).forEach((vp) => {
          ...
          if (vendor) {
            const existingLine = vendor.lines.find(
              (l) => (l.product_code === p.product_code || l.product_name === p.product_name)
            );
            if (!existingLine) {
              vendor.lines.push({
      ...
      vendorList.forEach((vendor) => {
        vendor.lines.forEach((line) => {
          const productKey = line.product_id || line.product || line.item_id || line.pr_line_id || line.product_name;
          if (!productMap[productKey]) {
            productMap[productKey] = {
              id: productKey,
              name: line.product_name || line.product?.name || line.item_name || line.description || 'Unknown Product',
      ...
      products.forEach((product) => {
        ...
        vendorList.forEach((vendor) => {
          const line = vendor.lines.find(
            (l) => (l.product_id || l.product || l.item_id || l.pr_line_id || l.product_name) === product.id
          );
          if (line) {
  ...
  // Get accepted vendor
  const acceptedVendorId = Object.keys(vendorDecisions).find((k) => vendorDecisions[k] === 'accept');
  const acceptedVendor = comparisonData.vendorList.find((v) => String(v.id) === String(acceptedVendorId));
    ...
    <>
      ...
      {error && !loading && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-6 text-center">
          ...
          <button onClick={loadDashboard} className="mt-3 text-sm text-red-600 underline hover:text-red-800">
            Try Again
          </button>
        </div>
      )}
          ...
          {prSummary && (
            <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                <span className="text-slate-600">{prSummary.warehouse_name || prSummary.warehouse || ''}</span>
                <span className="text-slate-400">|</span>
              ...
              {prItems.length > 0 && (
                <div className="mt-3 text-sm text-slate-600">
                  ...
                  {prItems.map((item) => `${item.product_name || item.description || 'Item'} (${item.quantity || item.qty || 0} ${item.uom || item.unit || ''})`).join(', ')}
                </div>
              )}
            </div>
          )}
          ...
          {comparisonData.vendorList.length === 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 mb-6 text-center">
              ...
              <p className="text-yellow-700 text-sm">
                No vendor quotes have been received for this Purchase Request yet. Please create RFQs and collect quotes first.
              </p>
              <button onClick={() => navigate('/purchase/rfq/new')} className="mt-4 px-4 py-2 bg-yellow-600 text-white rounded-lg text-sm font-medium hover:bg-yellow-700 transition">
                Create RFQ
              </button>
            </div>
          )}
            ...
            <div className="bg-white rounded-xl border border-slate-200 mb-6 shadow-sm">
              <div className="p-6 border-b border-slate-200">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {comparisonData.vendorList.length} vendor(s) &middot; {comparisonData.products.length} product(s)
                </p>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      {comparisonData.vendorList.map((vendor) => {
                        ...
                        return (
                          <th key={vendor.id} className={`text-center px-6 py-4 min-w-[200px] transition-colors ${
                            decision === 'accept' ? 'bg-green-50 border-t-4 border-t-green-500'
                            : decision === 'reject' ? 'bg-red-50 border-t-4 border-t-red-400 opacity-70' : ''
                          }`}>
                            <div className="font-semibold text-slate-800">{vendor.vendorName}</div>
                            {vendor.quoteNo && <div className="text-xs text-slate-500 mt-1">{vendor.quoteNo}</div>}
                          </th>
                        );
                      })}
                    </tr>
                  ...
                  <tbody>
                    {comparisonData.products.map((product) => (
                      <>
                        {/* Product header row */}
                        <tr key={`header-${product.id}`} className="bg-slate-100 border-b border-slate-200">
                          <td className="sticky left-0 z-10 bg-slate-100 px-6 py-3 font-semibold text-slate-800 border-r border-slate-200">
                            ...
                            {product.requestedQty && (
                              <span className="ml-2 text-xs text-slate-500 font-normal">(Req: {product.requestedQty} {product.uom})</span>
                            )}
                          </td>
                          {comparisonData.vendorList.map((vendor) => {
                            const decision = vendorDecisions[String(vendor.id)];
                            return (
                              <td key={`${product.id}-${vendor.id}-header`} className={`text-center px-6 py-3 ${
                                decision === 'accept' ? 'bg-green-50/50' : decision === 'reject' ? 'bg-red-50/30 opacity-70' : 'bg-slate-100'
                              }`} />
                            );
                          })}
                        </tr>
                        ...
                        {/* Detail rows */}
                        {['unit_price', 'quantity', 'gst', 'total', 'delivery', 'payment'].map((field) => (
                          <tr key={`${product.id}-${field}`} className="border-b border-slate-100 hover:bg-slate-50/50">
                            ...
                            {comparisonData.vendorList.map((vendor) => {
                              ...
                              const decision = vendorDecisions[String(vendor.id)];
                              const cellBg = decision === 'accept' ? 'bg-green-50/50' : decision === 'reject' ? 'bg-red-50/30 opacity-70' : '';

                              if (!line) {
                                return <td key={`${product.id}-${vendor.id}-${field}`} className={`text-center px-6 py-2.5 text-slate-400 ${cellBg}`}>--</td>;
                              }

                              const unitPrice = parseFloat(line.unit_price || line.price || 0);
                              const qty = parseFloat(line.quantity || line.qty || 0);
                              const gst = parseFloat(line.gst_percent || line.gst || line.tax_percent || vendor.gstPercent || 0);
                              const total = parseFloat(line.total || line.line_total || line.amount || unitPrice * qty || 0);
                              const delivery = line.delivery_days || line.delivery || vendor.deliveryDays || null;
                              const payment = line.payment_terms || vendor.paymentTerms || '';

                              const isBestPrice = field === 'unit_price' && unitPrice > 0 && unitPrice <= product.lowestPrice;
                              const isBestTotal = field === 'total' && total > 0 && total <= product.lowestTotal;

                              let cellContent = '--';
                              if (field === 'unit_price') cellContent = unitPrice > 0 ? formatCurrency(unitPrice) : '--';
                              else if (field === 'quantity') cellContent = qty > 0 ? `${formatNumber(qty, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ${product.uom}` : '--';
                              else if (field === 'gst') cellContent = gst > 0 ? `${gst}%` : '--';
                              else if (field === 'total') cellContent = total > 0 ? formatCurrency(total) : '--';
                              else if (field === 'delivery') cellContent = delivery ? `${delivery} days` : '--';
                              else if (field === 'payment') cellContent = payment || '--';

                              return (
                                <td key={`${product.id}-${vendor.id}-${field}`} className={`text-center px-6 py-2.5 ${cellBg}`}>
                                  <span className={`inline-flex items-center gap-1 ${isBestPrice || isBestTotal ? 'text-green-700 font-semibold' : 'text-slate-700'}`}>
                                    {cellContent}
                                    {(isBestPrice || isBestTotal) && <Star size={14} className="text-amber-500 fill-amber-500" />}
                                  </span>
                                </td>
                              );
                            })}
                    ...
                    <tr className="bg-slate-50 border-t-2 border-slate-300 font-semibold">
                      <td className="sticky left-0 z-10 bg-slate-50 px-6 py-4 text-slate-800 border-r border-slate-200">TOTAL VALUE</td>
                      {comparisonData.vendorList.map((vendor) => {
                        const total = parseFloat(vendor.totalValue || 0);
                        const isBest = total > 0 && comparisonData.bestTotalValue && total <= comparisonData.bestTotalValue;
                        const decision = vendorDecisions[String(vendor.id)];
                        return (
                          <td key={`total-${vendor.id}`} className={`text-center px-6 py-4 ${
                            decision === 'accept' ? 'bg-green-50' : decision === 'reject' ? 'bg-red-50/30 opacity-70' : ''
                          }`}>
                            <span className={`inline-flex items-center gap-1 ${isBest ? 'text-green-700' : 'text-slate-800'}`}>
                              {total > 0 ? formatCurrency(total) : '--'}
                              {isBest && <Star size={14} className="text-amber-500 fill-amber-500" />}
                            </span>
                          </td>
                        );
                      })}
                    ...
                    <tr className="border-t-2 border-slate-200">
                      <td className="sticky left-0 z-10 bg-white px-6 py-4 font-semibold text-slate-800 border-r border-slate-200">Decision</td>
                      {comparisonData.vendorList.map((vendor) => {
                        const vendorId = String(vendor.id);
                        const decision = vendorDecisions[vendorId];
                        return (
                          <td key={`decision-${vendor.id}`} className="text-center px-6 py-4">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => handleDecision(vendor.id, 'accept')} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition ${
                                decision === 'accept' ? 'bg-green-600 text-white shadow-md ring-2 ring-green-300' : 'bg-green-50 text-green-700 border border-green-300 hover:bg-green-100'
                              }`}>
                                <CheckCircle2 size={16} /> Accept
                              </button>
                              <button onClick={() => handleDecision(vendor.id, 'reject')} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition ${
                                decision === 'reject' ? 'bg-red-600 text-white shadow-md ring-2 ring-red-300' : 'bg-red-50 text-red-700 border border-red-300 hover:bg-red-100'
                              }`}>
                    ...
                    <tr className="border-t border-slate-200">
                      <td className="sticky left-0 z-10 bg-white px-6 py-4 font-semibold text-slate-800 border-r border-slate-200">Notes</td>
                      {comparisonData.vendorList.map((vendor) => {
                        const vendorId = String(vendor.id);
                        return (
              ...
              {acceptedVendor ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 flex-wrap text-sm">
                    ...
                    <span className="font-semibold text-green-700 bg-green-50 px-3 py-1 rounded-full">{acceptedVendor.vendorName}</span>
                  </div>
                    ...
                    <span className="flex items-center gap-1.5">
                      ...
                      {acceptedVendor.gstPercent > 0 && (
                        <span className="text-slate-500">
                          + {acceptedVendor.gstPercent}% GST = <span className="font-semibold text-slate-800">{formatCurrency(parseFloat(acceptedVendor.totalValue) * (1 + parseFloat(acceptedVendor.gstPercent) / 100))}</span>
                        </span>
                      )}
                    </span>
                ...
                <p className="text-slate-500 text-sm">No vendor selected yet. Use the Accept button above to select a vendor.</p>
              )}

              {/* Auto-generate PO checkbox */}
              <div className="mt-5 pt-5 border-t border-slate-200">
                <label className="inline-flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={autoGeneratePO} onChange={(e) => setAutoGeneratePO(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-primary-600 focus:ring-primary-500" />
                  <span className="text-sm text-slate-700 font-medium">Auto-generate Purchase Order</span>
                </label>
              </div>

              {/* Action buttons */}
              <div className="mt-5 flex items-center gap-3">
                <button onClick={() => handleSubmit(true)} disabled={submitting} className="flex items-center gap-2 px-5 py-2.5 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition disabled:opacity-50">
                  {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                  Save Draft
                </button>
                <button onClick={() => handleSubmit(false)} disabled={submitting || !acceptedVendorId} className="flex items-center gap-2 px-6 py-2.5 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {createdPO && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 mb-6 shadow-sm">
              ...
              <p className="text-green-700 text-sm mb-4">
                {createdPO.po_no || createdPO.po_number || 'PO'} created for {acceptedVendor?.vendorName || createdPO.vendor_name || 'selected vendor'}
              </p>
              <div className="flex items-center gap-3">
                <button onClick={() => navigate(`/purchase/orders/${createdPO.id || createdPO.po_id}`)} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition">
                  <ChevronRight size={16} /> View PO
                </button>
                <button onClick={() => window.open(`/api/purchase/orders/${createdPO.id || createdPO.po_id}/download/`, '_blank')} className="flex items-center gap-2 px-4 py-2 border border-green-300 text-green-700 rounded-lg text-sm font-medium hover:bg-green-100 transition">
      ...
      {!dashboardData && !loading && !error && (
        <div className="bg-white rounded-xl border border-slate-200 p-16 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-slate-700 mb-2">Select a Purchase Request to Begin</h3>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            Choose a Purchase Request from the dropdown above and click "Load" to view and compare vendor quotes side-by-side.
          </p>
        </div>
      )}
    </>
