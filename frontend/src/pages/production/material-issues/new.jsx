...
export default function CreateMaterialIssue() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await apiClient.post('/api/production/material-issues/', cleanFormData(formData));
      toast.success('Material Issue created successfully!');
      navigate('/production/material-issues');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Failed to create Material Issue');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <MainLayout>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Material Issues', path: '/production/material-issues' },
          { label: 'New' },
        ]}
      />
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <h3 className="text-lg font-semibold text-slate-800 mb-4 pb-2 border-b">Material Issue Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Issued By <span className="text-red-500">*</span></label>
                <select name="issued_by" value={formData.issued_by} onChange={handleChange} required className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
