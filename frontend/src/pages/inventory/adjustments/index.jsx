...
export default function StockAdjustmentList() {
  ...
  const filters = [
    { key: 'search', label: 'Search', type: 'text', placeholder: 'Search adjustments...' },
    {
      key: 'warehouse',
      label: 'Warehouse',
      type: 'select',
  ...
  const columns = [
    { key: 'adjustment_no', label: 'Adjustment No', sortable: true },
  ...
  const filteredData = (data || []).filter((row) => {
    ...
    if (filterValues.adjustment_type && row.adjustment_type !== filterValues.adjustment_type) {
      return false;
    }
    if (filterValues.approval_status && row.approval_status !== filterValues.approval_status) {
      return false;
    }
    if (filterValues.date_from && row.adjustment_date < filterValues.date_from) {
      return false;
    }
    return true;
  });
