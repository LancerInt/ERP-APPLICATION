...
export default function JobWorkDCList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <button
          onClick={(e) => { e.stopPropagation(); navigate(`/inventory/job-work-dc/${row.id}`); }}
    ...
    <MainLayout>
      <PageHeader
        ...
        actions={{
          ...
          onExport: () => {},
          ...(canCreate('Job Work') ? { createLink: '/inventory/job-work-dc/new', createLabel: 'New JW DC' } : {}),
        }}
      />
      {showFilters && (
        <FilterPanel
